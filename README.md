...
- 問題は **KANJIDIC2**（漢字の音訓読み）と **JMdict**（語彙と読み）から自動生成。
  誤答選択肢（ダミー）は**「間違い」なら同格**とみなし、同じ漢字の別読み・連濁ミスなどの
  誤読候補と同級プールの実在読みを混ぜてランダムに3つ選びます
  （正答と同語の別読み・ローマ字で同一になる読みは除外）。
...
# - placeファイル: 全サーバーモジュール・問題データ・SpawnLocationが入っていること
#   (ビルド漏れでサーバーが起動せずキャラクターが落ち続ける事故の防止)
